...
class ContentDBSitemapGenerator(BaseSitemapGenerator):
    ...
    def add_route_to_urlset(self, urlset: ET.Element, draft: Draft, language_iso2: str):
    ...
    def dump_drafts_to_file(self, drafts: list[Draft], filepath: str, language_iso2: str):
        urlset = self.create_base_urlset()

        for draft in drafts:
            self.add_route_to_urlset(urlset, draft, language_iso2)
        if not urlset:
    ...
    def process_drafts_batch(self, current_drafts: list[Draft], language_iso2: str, file_counter: int):
        if self.sitemap_channel.merge_languages_in_sitemap:
            merged_key = f"{CONTENTDB_PACKAGE}-{file_counter}"
            ...
            for route in current_drafts:
                self.add_route_to_urlset(self.merged_urlsets[merged_key], route, language_iso2)
        else:
            self.dump_drafts_to_file(current_drafts, f"{CONTENTDB_PACKAGE}-{file_counter}", language_iso2)

    def process_drafts(self, drafts_to_process: list[Draft], language_iso2: str):
        ...
        for drafts in tqdm(drafts_to_process, total=total_drafts, desc="Processing drafts", disable=None):
            ...
            if len(current_drafts) >= settings.LIMIT:
                self.process_drafts_batch(current_drafts, language_iso2, file_counter)
    ...
    def generate(self, **kwargs):
        ...
        try:
            for language_iso2 in self._get_shop_languages():
                ...
                self.process_drafts(drafts, language_iso2)
